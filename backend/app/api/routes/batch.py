...
import schemas.batch as schemas
from crud.batch import batch as crud

router = APIRouter()

@router.get("/api/batch", response_model=List[schemas.BatchSchema])
async def list_batch(db: Session = Depends(get_db), skip: int = 0, limit: int = 100) -> List[schemas.BatchSchema]:
...
async def create_batch(request: schemas.BatchCreateSchema, db: Session = Depends(get_db)) -> schemas.BatchSchema:
    is_batch_exists = dependency.get_that_by_this(key="name", value=request.name, db=db, model=Batch)
    if not is_batch_exists:
        batch = crud.create(db=db, new_batch=request)
        return batch
...
async def delete_batch(id: int, db: Session = Depends(get_db)) -> schemas.BatchSchema:
   is_batch_exists = dependency.get_that_by_this(key="id", value=id, db=db, model=Batch)
   if is_batch_exists:
       batch = crud.delete(db=db, id=id)
       return batch
